...
should_parse_utf8_binary__test() ->
  ...
  ?assertEqual({<<"TopicA">>, <<>>}, mqtterl_codec:parse_utf8(Raw1)),
  Raw2 = <<0, 5, 104, 101, 108, 108, 111>>,
  ?assertEqual({<<"hello">>, <<>>}, mqtterl_codec:parse_utf8(Raw2)),
  Raw3 = <<0, 0>>,
  ?assertEqual({<<>>, <<>>}, mqtterl_codec:parse_utf8(Raw3)).
...
should_parse_remaining_length__test() ->
  {RL1, _} = mqtterl_codec:parse_remaining_length(<<0:1, 123:7, 0:1, 2:7>>),
  ?assertEqual(123, RL1),
  {RL2, _} = mqtterl_codec:parse_remaining_length(<<1:1, 65:7, 0:1, 2:7>>),
  ?assertEqual(321, RL2).
...
should_parse_connect_packet__test() ->
  ...
  {Type, _Flags, Remaining1} = mqtterl_codec:parse_type(Packet),
  ?assertEqual(?CONNECT, Type),

  {RemainingLength, Remaining2} = mqtterl_codec:parse_remaining_length(Remaining1),
  ?assertEqual(RemainingLength, 22),

  {Header, Remaining3} = mqtterl_codec:parse_connect_variable_header(Remaining2),
  ...
  {Payload, Remaining4} = mqtterl_codec:parse_connect_payload(Header, Remaining3),
